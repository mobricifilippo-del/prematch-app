...
const DATA = {
  ...
  // SOLO calcio per le società demo (come richiesto)
  clubsByLeague: {
  ...
  clubProfiles: {
    "ASD Roma Nord": demoClub("ASD Roma Nord"),
    "Sporting Tuscolano": demoClub("Sporting Tuscolano"),
    "Virtus Marino": demoClub("Virtus Marino"),
    "Borghesiana FC": demoClub("Borghesiana FC"),
    "Atletico Ostia": demoClub("Atletico Ostia"),
    "Accademia Ragazzi": demoClub("Accademia Ragazzi"),
    "Junior Sporting": demoClub("Junior Sporting"),
  }
};
function demoClub(name){
  return {
    logo: LOGOS.icon,
...
function gridCard(item, onClick){
  ...
  card.addEventListener("click", ()=>{
    card.classList.add("flash");          // effetto visivo evidente
    setTimeout(()=>{ onClick(); }, 220);  // piccola pausa prima di avanzare
  });
  return card;
}
...
function pageClubProfile(clubName){
  ...
  const club = DATA.clubProfiles[clubName] || demoClub(clubName);

  app.appendChild(sectionTitle(clubName, `${state.league||""} • ${state.gender||""} • ${state.region||""}`));

  // header: logo + pulsante tondo (uguali dimensioni)
  ...
  // Accordion: Info / Gallery / Match in programma
  const acc = h("div",{class:"container acc"});
  acc.appendChild(accItem("Informazioni", [
    pRow("Email", club.contacts.email),
    pRow("Telefono", club.contacts.tel),
    pRow("Impianto", "Via dello Sport 1, Roma (esempio)"),
  ]));
  acc.appendChild(accItem("Gallery foto", galleryContent(club.gallery)));
  acc.appendChild(accItem("Match in programma", matchesContent(club.matches)));
  app.appendChild(acc);
}

function pRow(label, value){
  const rw = h("div",{class:"row"},[
    h("div",{class:"team"}, label),
    h("div",{class:"meta"}, value||"—")
  ]);
  return rw;
}
...
function accItem(title, bodyEl){
  ...
  if (Array.isArray(bodyEl)) bodyEl.forEach(el=>bd.appendChild(el));
  else bd.appendChild(bodyEl);
...
/* ----- Bottone tondo PreMatch (uguale al logo) ----- */
function createPmRoundButton(){
  const wrap = h("div");
  const btn = h("button",{class:"pm-round", title:"Crea PreMatch"},[
    h("img",{src:LOGOS.icon, alt:"PM"})
  ]);
  btn.addEventListener("click", openPrematchModal);
  const label = h("div",{class:"pm-cta"}, "Crea PreMatch");
  wrap.appendChild(btn); wrap.appendChild(label);
  return wrap;
}

/* ----- Modale PreMatch (solo maglia + data/ora + luogo + tipo) ----- */
function openPrematchModal(){
  ...
  bd.appendChild(tipoLbl);
  bd.appendChild(tipoSel);
  bd.appendChild(clrLbl);
